...
def main():
    ...
    excluded_vehicles = {s_v["v_2"]["vehID"], s_v["v_3"]["vehID"], s_v["v_4"]["vehID"], s_v["v_1"]["vehID"],
                         s_v["v_5"]["vehID"], s_v["v_6"]["vehID"], s_v["v_7"]["vehID"]}
    rag_result = []
        ...
        for step in range(3600):
            ...
            if all_stop == 0:
                ...
                if current_time == 13.0:
                    traci.vehicle.setStop(s_v["v_5"]["vehID"], s_v["v_5"]["edgeID"], pos=s_v["v_5"]["pos"],
                                          laneIndex=s_v["v_5"]["laneIndex"], duration=s_v["v_5"]["duration"], flags=0)
                if current_time == 16.0:
                    traci.vehicle.setStop(s_v["v_6"]["vehID"], s_v["v_6"]["edgeID"], pos=s_v["v_6"]["pos"],
                                          laneIndex=s_v["v_6"]["laneIndex"], duration=s_v["v_6"]["duration"], flags=0)
                if current_time == 19.0:
                    traci.vehicle.setStop(s_v["v_7"]["vehID"], s_v["v_7"]["edgeID"], pos=s_v["v_7"]["pos"],
                                          laneIndex=s_v["v_7"]["laneIndex"], duration=s_v["v_7"]["duration"], flags=0)
            ...
            if current_time < 200:
                ...
                if current_time == next_switch_time:
                    if is_stop == 0:
                        ...
                        stay_time = car_removing(phase_5, net, time_threshold, dist_threshold, current_time, stay_time,
                                                 excluded_vehicles)
                        pressure_llama = llm_suggestion(phase_5, traffic_event, traffic_language, input, format,
                                                        strategy)
